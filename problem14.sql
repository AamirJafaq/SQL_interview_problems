...
-- Insert sample date into the table.
...
/* Write an SQL query to find the average trip duration for each driver in each city excluding 
trips where the rating is below 3. The result should include driver_id, city, and avg_trip_duration 
rounded to two decimal places. 
Note: The avg_trip_duration is calculated as the average of the differences between end_time 
and start_time for each trip.
*/
SELECT driver_id, city, ROUND(avg(EXTRACT(EPOCH FROM (end_time-start_time))/60)) AS avg_trip_duration
FROM trips
WHERE rating>=3
GROUP BY 1, 2
